...
## Getting Started
- Pull the repo.
- Copy `.env.example` to `.env.development.local`. Don't commit `.env.development.local`. It's in `.gitignore` for a reason.
- Set the `REACT_APP_CLIENT_ID` and `REACT_APP_CLIENT_SECRET` variables in `.env.development.local`.
- Run `yarn install`
- Run `yarn start`
